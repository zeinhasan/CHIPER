...
### Added
- **Test Suite** — Added a `pytest` suite under `tests/` covering all features: domain filtering, SSRF prevention (IPv4/IPv6, schemes, blocked hosts), URL normalization & link extraction, sitemap parsing, circuit breaker, task store + inflight limit, request-schema validation & per-page config, API-key middleware, PDF detection/extraction, summary-prompt sanitization, and persistent-history CRUD. Optional-dependency tests self-skip when their deps are absent. (`tests/`, `pytest.ini`, `requirements.txt`)
